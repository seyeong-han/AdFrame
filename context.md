...
- Apple infographic preset implementation lives in `app-web/lib/tldraw/templates.ts` as `composeAppleInfographicTemplate()`. As of 2026-06-28 it follows the user's hand-drawn dense tile-board: two large top asset tiles; a second row with a large left `apple-hero` main tile and a right-side price/asset stack; a middle row with left asset, two stacked copy cards, and right asset; and a bottom row with left mini copy/asset stack, dominant center asset, and right asset/copy stack. It uses a light `ff-canvas-bg` variant plus `paper`/`orange` card tones. Asset organization uses metadata scoring helpers (`describeAsset`, `scoreAssetForSlot`, `pickHeroAsset`, `pickAssetForSlot`) over current `asset.name`/`asset.alt`/`kind`, with unique assets preferred and fallback reuse when the sketch has more slots than extracted images.
...
- `ProductAsset.bgRemoved` is a required boolean. Live extraction normalizes it: assets under `/generated/removebg/` or named `(bg removed)` are `true`, section/video/upload assets are `false`. `ff-cutout-image` sizing splits by `bgRemoved`: remove.bg cutouts use `fitImageToBox()` (`min(boxW/naturalW, boxH/naturalH)` — shorter dimension wins, no forced full width); all other assets use `fitImageFullWidth()` (`width = boxW`, height scales proportionally up to `boxH`). Template slots must preserve the asset's actual `bgRemoved` value instead of hardcoding `false`, because fallback reuse can place cutouts into section-like tile slots. Explicit `shape.props.w/h` box sizing avoids broken `%` max-height under tldraw zoom transforms.
