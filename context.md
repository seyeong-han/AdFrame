...
- Default editor composition now uses a role-based "hero + feature mosaic" layout: hero/gallery assets are used for the central product cutout, `kind: "section"` visual crops are matched to feature cards and placed as cover-fit visual strips, an ambient section panel fills the upper right, and video assets are intentionally excluded from static canvas placement.
...
- Template composition regression: `npm run test:template-composition` asserts the canvas template includes at least three section visual image shapes, keeps video assets out of static image shapes, and parents every export shape to the frame.
